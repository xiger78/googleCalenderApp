...
- 점심·저녁 시간을 변경한 뒤, 저녁시간 입력란 아래 **저장** 버튼(전체 폭)을 눌러 저장
- 저장된 휴계시간은 이력확인·저장 미리보기·CSV 출력 시 가동시간 계산에서 합산 제외

#### 5-3. 근태장표출력(CSV)
- 출력할 달 선택
- **출력** 버튼으로 CSV 파일 생성 및 공유 (§5-2 휴계시간 설정 반영)
...
| 휴계시간 설정 | **휴계시간설정(점심,저녁)** 카드 — 점심 기본 1시간·저녁 0시간, **저장** 버튼으로 반영; **근태장표출력(CSV)** 카드 분리 |
