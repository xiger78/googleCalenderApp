...
- 가동시간 = 퇴근 시각 − 출근 시각 − **설정의 점심시간**, 괄호 안에 소수로 표시 (9시간 → `(9.0)`, 9시간 30분 → `(9.5)`)
- 예:
```
[가동시간:160.0]
...
- 괄호 안 가동시간은 저장 미리보기와 동일 (점심시간 제외, `9.0` / `9.5` 형식)
- 예:
```
[가동시간:160.0]
2026/06/03(수) 09:00-18:00 (8.0)
...
| 가동시간 표시 | 이력·저장 미리보기에 출퇴근 시각 뒤 **(9.0)** / **(9.5)** 형식 가동시간 (점심시간 제외) |
