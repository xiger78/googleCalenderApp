...
表示言語、休憩時間、勤怠表出力(CSV)、メール送信の設定を行います。
...
- 昼食・夕食の時間を変更後、夕食入力欄下の **保存** ボタン（全幅）で保存
- 保存した休憩時間は履歴確認・保存プレビュー・CSV 出力の稼働時間計算から合算除外

#### 5-3. 勤怠表出力(CSV)
- 出力する月を選択
- **出力** で CSV ファイルを生成・共有（§5-2 の休憩時間設定を反映）
...
| 稼働時間表示 | 履歴・保存プレビューに出退時刻の後 **(9.0)** / **(9.5)** 形式の稼働時間（昼食・夕食休憩除外） |
| 休憩時間設定 | **休憩時間設定(昼食・夕食)** カード — 昼食初期1時間・夕食0時間、**保存** ボタンで反映; **勤怠表出力(CSV)** カードを分離 |
